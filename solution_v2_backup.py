...
def augment(xb):
    n = xb.shape[0]
    c = 0.65 + 0.70 * torch.rand(n, 1, 1, 1, device=xb.device)
    b = 0.18 * torch.randn(n, 1, 1, 1, device=xb.device)
    xb = xb * c + b
...
def build_inputs(arr, gm, gs, hps):
    t = torch.tensor((arr - gm) / gs, device=DEV).unsqueeze(1)
    hp = t - F.avg_pool2d(t, 9, 1, 4)
    hp = hp / hps
    return torch.cat([t, hp], 1)
...
def main():
    ...
    GM = float(Xtr_all.mean()); GS = float(Xtr_all.std()) + 1e-6

    with torch.no_grad():
        tt = torch.tensor((Xtr_all - GM) / GS, device=DEV).unsqueeze(1)
        hp_std = float((tt - F.avg_pool2d(tt, 9, 1, 4)).std()) + 1e-6
        del tt
        if DEV == 'cuda':
            torch.cuda.empty_cache()
    ...
    Xg = build_inputs(Xtr_all, GM, GS, hp_std)
    Xteg = build_inputs(Xte, GM, GS, hp_std)
