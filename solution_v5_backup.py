...
class HeadNet(nn.Module):
    def __init__(self, backbone, fdim, mode):
        super().__init__()
...
def build_model():
    try:
        ...
        bb = tv.models.convnext_tiny(weights='DEFAULT')
        return HeadNet(bb, 768, 'tv'), True
    except Exception as e:
        print('convnext unavailable:', str(e)[:80], flush=True)
    try:
        import torchvision as tv
        bb = tv.models.resnet18(weights='DEFAULT')
        feats = nn.Sequential(*list(bb.children())[:-2])
        return HeadNet(feats, 512, 'seq'), True
    except Exception as e:
        print('resnet unavailable, from-scratch fallback:', str(e)[:80], flush=True)
    return HeadNet(ScratchBackbone(3), 256, 'seq'), False
...
def train_fold(Xtr, ytr, Xva_list, epochs=12, bs=64):
    model, is_pre = build_model()
    model = model.to(DEV)
    lr = 3e-4 if is_pre else 1.3e-3
...
def main():
    ...
    for f in range(nf):
        ...
        out_va, out_te = train_fold(Xg[ti], yg[ti], [Xg[vi], Xteg], epochs=12)
        oof[va] = out_va
        test_pred += out_te / nf
