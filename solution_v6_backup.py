...
BACKBONES = [
    {'name': 'convnext_tiny', 'kind': 'tv', 'fdim': 768, 'res': 128, 'lr': 3e-4, 'ep': 12},
    {'name': 'vit_small_patch14_dinov2.lvd142m', 'kind': 'dino', 'fdim': 384, 'res': 112, 'lr': 1.5e-4, 'ep': 10},
]
...
class HeadNet(nn.Module):
    def __init__(self, backbone, fdim, mode, res):
        ...
        self.mode = mode
        self.res = res
        hin = fdim if mode == 'vit' else fdim * 2
    ...
    def forward(self, x):
        ...
        if self.mode == 'vit':
            ...
            f = self.backbone.features(x) if self.mode == 'tv' else self.backbone(x)
            a = F.adaptive_avg_pool2d(f, 1).flatten(1)
            m = F.adaptive_max_pool2d(f, 1).flatten(1)
            pooled = torch.cat([a, m], 1)
        h = self.drop(F.relu(self.fc1(self.drop(pooled))))
...
def build_model(spec):
    ...
    if kind == 'dino':
        try:
            import timm
            bb = timm.create_model(spec['name'], pretrained=True, num_classes=0, dynamic_img_size=True)
            return HeadNet(bb, spec['fdim'], 'vit', spec['res']), True
        except Exception as e:
            print(f'  dino unavailable ({str(e)[:60]}) -> efficientnet_b0', flush=True)
            try:
                import torchvision as tv
                return HeadNet(tv.models.efficientnet_b0(weights='DEFAULT'), 1280, 'tv', 128), True
            except Exception as e2:
                return HeadNet(ScratchBackbone(3), 256, 'seq', 128), False
    try:
        import torchvision as tv
        bb = getattr(tv.models, spec['name'])(weights='DEFAULT')
        return HeadNet(bb, spec['fdim'], 'tv', spec['res']), True
    except Exception as e:
        print(f"  {spec['name']} unavailable ({str(e)[:60]}) -> from-scratch", flush=True)
...
def main():
    ...
    ens_oof_rank = np.zeros(n_tr); ens_test_rank = np.zeros(n_te); nmemb = 0
    for spec in BACKBONES:
        ...
        for f in range(nf):
            ...
            print(f"[{spec['name']}] fold {f} start", flush=True)
            out_va, out_te = train_fold(spec, Xg[ti], yg[ti], [Xg[vi], Xteg])
            oof[va] = out_va
            test_pred += out_te / nf
        sc = resid_corr(oof, y, bright)
        print(f"[{spec['name']}] OOF resid-corr {sc:.4f}", flush=True)
        ens_oof_rank += rankdata(oof); ens_test_rank += rankdata(test_pred); nmemb += 1

    ens_oof_rank /= nmemb; ens_test_rank /= nmemb
    print(f'ENSEMBLE OOF resid-corr {resid_corr(ens_oof_rank, y, bright):.4f}', flush=True)

    ysort = np.sort(y)
    tr_ranks = rankdata(ens_test_rank) - 1
    pct = tr_ranks / (n_te - 1)
    pos = pct * (len(ysort) - 1)
